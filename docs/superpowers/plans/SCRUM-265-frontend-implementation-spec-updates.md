...
| **WI-F** | SDUI OPEN sends the route **with query string** (`?basisId=` reaches `applyBasisFill`) | **SDUI** | 1 edited (+ test) |

Isolation reminder: WI-B/WI-C/WI-F are SDUI-only; WI-D lives in the shared `workspace-tabs` feature and must
...
### WI-F — SDUI OPEN sends the route with query string

**Zone:** SDUI. **File:** edit `src/features/sdui/ui/sdui-screen.tsx` (the main effect builds
`const route = location.pathname` — ~line 55); test alongside.
**Backend contract:** §2 (`?basisId=`). Server-side consumer: `ViewController.applyBasisFill` reads
`?basisId` **from the `route` field of the OPEN request** — not from anything else.

**Intent.** WI-E fixes the legacy path; this fixes the symmetric hole in the SDUI path. Today the
primary OPEN dispatch sends `route: location.pathname` — the query string is dropped, so for a
`newView` receiver the server never sees `?basisId` and opens an empty form. The EVENT dispatch
already does it right (`src/features/sdui/lib/dispatch.ts` sends
`location.pathname + location.search`) — OPEN is the odd one out.

*Status note:* latent as of 2026-07-13 — the only wired basis receiver (СчётКОплате) opens via the
legacy form, so WI-E alone fixes the visible bug. WI-F becomes load-bearing the moment any
basis-receiver type flips to `newView`. Diagnosis: `READMEs/fin-web/SCRUM-265-basis-fill-blank-fields-fix.md`.

**Change** in `sdui-screen.tsx`:
1. In the OPEN path, send `route = location.pathname + location.search` (mirror `dispatch.ts`).
2. **Keep client-side keys on `location.pathname`**: the SDUI cache (`sdui-cache-store`) and the
   dirty/tab bookkeeping key by route today — a `?basisId=…` in the key would break tab
   restore/session reuse for the same document. Only the wire `route` gains the query string;
   cache/tab keys stay pathname-based. Audit the other `location.pathname` usages in the same effect
   before touching them — most are keys, not wire payloads.

**Acceptance:** opening a `newView` route `…/new?basisId=<id>` produces an OPEN request whose `route`
contains `?basisId=<id>` (assert on the transport payload); plain document opens and workspace-tab
restore are unchanged; language re-OPEN still works.

**Tests:** unit-test the OPEN payload includes the search string when present and omits nothing when
absent; regression: cache store save/get still keyed without the query.
...
fix:  send query string in SDUI OPEN route (basisId)            # WI-F
```
