...
Порядок реализации: **0 → 1 → 2 → 3 → 4**. Слой 4 в Phase 2 сводится к типу `tab` +
инфраструктуре для catch-all; снятие регексов отложено (см. ниже).
...
### Слой 4 — §4.5 понижение `resolve-page-type` — СНЯТИЕ РЕГЕКСОВ ОТЛОЖЕНО

**Находка на этапе планирования:** снятие регексов `document-entry`/
`dictionary-entry`/`module` в текущем состоянии фронта **ломает легаси-вкладки** и в
Phase 2 безопасно не выполнимо:

- `ModulePage` (`/modules/:pageCode`) — **не SDUI**, `/api/view` не дёргает вовсе →
  `tab.kind==='MODULE'` фронту не приходит, заменить регекс нечем. Разблокируется
  shell-миграцией (задача 7 «shell», отдельный тикет).
- Регексы `document-entry`/`dictionary-entry` — path-based, обслуживают **и SDUI, и
  легаси** вариант одного URL (развилка по `newView`). По §3 бэк-спеки `newView=true`
  — меньшинство из ~93 типов; большинство карточек легаси, у них нет view-ответа, и
  вкладку им создаёт этот регекс. Снять → сломать вкладки большинства.

**Решение по слою 4 в Phase 2:**

- В `ViewResponse` добавить `tab?: { kind; icon?; closable? }` (сейчас поля нет).
- Инфраструктуру `tab.kind` (`mapKindToPageType`, проп `onTab` в `SduiScreen`)
  завести, но потребитель — **только catch-all** (там конфликта с регексами нет:
  catch-all ловит маршруты, которые регекс не матчит).
- Регексы `resolve-page-type` **оставить как есть** (легаси-вкладки целы). Авторство
  вкладок SDUI-страницами не добавлять — при живых регексах избыточно и задвоило бы
  `activateOrCreate`. Добавить регресс-гард + маркер отсрочки.
- Критерий §7.7 (MODULE из `tab.kind`) — **заблокирован shell-миграцией**, не Phase 2.
  Отписать бэкендеру (§8 бэк-спеки).
