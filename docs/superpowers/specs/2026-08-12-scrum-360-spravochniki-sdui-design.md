...
**Резолюция Q-2 (v3-back, 2026-08-28): `_level` не будет.** Иерархия — drill-down по ADR-0025
(проваливание в папку, крошки в TOOLBAR, глиф через `cellKind=ICON`+`iconMap`); плоское дерево с
отступами — конфликтующая модель отображения, отдельная задача владельца. `HIERARCHY`-ветка
остаётся no-op-фолбэком (не ломается без данных), отступы не рисуем и данных под них не ждём.

**Проверка.** Иерархический справочник в списке показывает группы/элементы визуально различимо
(глифы), навигация — drill-down.
...
2. **Props-only фолбэк** `allowCreate ?? canBrowse` (`reference-field-node.tsx:221`) снимаем
   **только после** подтверждения бэком гейта C1.4 `RefActionsCompletenessIT` (вопрос Q-1) — до
   ...
   SCRUM-291 §18.6).
   **Резолюция Q-1 (v3-back, 2026-08-28): гейт по коду выполнен** (резолвер эмитит `ref.showAll`
   безусловно на шапке документа, на справочнике — при включённом `dictionary-card=true`);
   эмпирический прогон `RefActionsCompletenessIT` — за нашей стороной, на **населённой** БД
   (в пустом контейнере тест падает на предусловии по конструкции):
   `BDD_USE_LOCAL_POSTGRES=true ./mvnw test -pl webbuh-api -am -Pit -Dtest=RefActionsCompletenessIT`.
   Зелёный → снимаем фолбэк; красный со списком нарушений → возвращаем бэку.
...
5. **Риск `?copyFrom`** (SCRUM-217: копия остаётся на легаси-форме): при снятии роутов запрос
   ...
   ответу бэка).
   **Резолюция Q-4 (v3-back, 2026-08-28): снимать можно.** Для `new_view=FALSE` 422 гарантирован
   (`SCREEN_NOT_SDUI`); для `new_view=TRUE` бэк применяет `?copyFrom` полностью — шапка + строки
   всех ТЧ (правка `DocumentCopyScratchBuilder`: свежие `rowId` против anti-forgery, патч
   `setValue` на OPEN). Контракт ответа не менялся — фронт-кода не требуется.
   **Мерж-гейт:** выкат бэка с этой правкой на dev **до** снятия роутов; перед мержем прогнать
   чек-лист приёмки v3 §4 (копирование документа с ТЧ: SDUI-карточка, строки видны сразу,
   источник не переписан).
