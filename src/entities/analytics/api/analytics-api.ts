...
/** Потолок ожидания ответа ассистента: два вызова модели подряд + валидация. */
  ...
  generate: (
    ...
    apiService
      .post<ApiResponse<AnalyticsGenerateResponse>>({
        ...
        data: request,
        signal,
        // Свой потолок вместо общих 60 секунд: за один вызов модель отрабатывает
        // дважды (выбор витрин, затем генерация), и на сложном вопросе это
        // спокойно занимает больше минуты. Значение согласовано с серверным
        // analytics.llm.read-timeout-seconds=180 плюс запас на валидацию и пробные
        // запросы — клиент не должен сдаваться раньше сервера.
