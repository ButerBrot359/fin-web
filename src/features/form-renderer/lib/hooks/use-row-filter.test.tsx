...
describe('useRowFilter (отбор пикера по ячейке строки ТЧ)', () => {
  ...
  it('зарезервированный ключ parent игнорируется (parent — не af, а ?parent=)', () => {
    const { result } = renderRowFilter(
  ...
  it('parent рядом с af-ключом → af остаётся, parent выкинут', () => {
    const { result } = renderRowFilter(
      { parent: 'OsnovnoeSredstvo', SchetUcheta: 'SchetUcheta' },
      {
        OsnovnyeSredstva: [
          { OsnovnoeSredstvo: { id: 1 }, SchetUcheta: { id: 2 } },
        ],
      },
    )
    expect(result.current).toEqual({ af: 'SchetUcheta:2' })
  })
})
