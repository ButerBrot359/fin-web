...
export const ReferenceFieldNode: FC<NodeProps> = ({ node }) => {
  ...
  const commonInputProps = {
    ...
    error: !!f.error,
    // Пояснение под полем — тот же props.helperText, что у числового поля и флажка.
    // Через него сервер показывает справку по выбранному значению: например, остаточную
    // стоимость исходного актива рядом с полем его выбора (обращение 21.09.2026).
    helperText: node.props?.helperText as string | undefined,
    loading,
