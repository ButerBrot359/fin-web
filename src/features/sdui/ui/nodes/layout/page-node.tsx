  ...
  return (
    <div
      className={cn(
        ...
        // Растянутой карточке нужна СОБСТВЕННАЯ прокрутка. Высота у неё жёсткая
        // (flex-1 от окна), а у табличной части внутри есть пол высоты — и когда
        // окна не хватает, бокс ТЧ выходил за вкладку и рисовался ПОВЕРХ итогов и
        // подвала («таблица накладывается друг на друга», Авансовый отчёт,
        // 10.09.2026): переполнение flex-колонки с фиксированной высотой не
        // увеличивает прокрутку `main`, а просто вытекает наружу. С этой
        // прокруткой при нехватке места карточка прокручивается, а подвал
        // остаётся ниже таблицы — как в 1С. Когда места достаточно, ничего не
        // переполняется и полоса не появляется.
