...
interface DecisionMessage {
  ...
  granted: boolean
  /**
   * Что показывает управляемая сторона: `browser` — вкладку, `window` — окно, `monitor` — экран
   * целиком.
   *
   * <p>Нужно агенту, чтобы понимать, попадает ли он туда, куда целится. Координаты
   * пересчитываются в область просмотра управляемой страницы, и это верно ровно тогда, когда
   * показана сама вкладка. При показе окна картинка включает ещё и рамку браузера с адресной
   * строкой, при показе экрана — вообще всё: указатель уедет на высоту этой обвязки. Молча
   * промахиваться в бухгалтерии нельзя, поэтому агент об этом предупреждается.
   */
  surface?: string
}
