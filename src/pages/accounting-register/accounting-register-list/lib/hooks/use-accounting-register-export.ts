...
export const useAccountingRegisterExport = (
  ...
  return useCallback(
    async (entries: AccountingRegisterEntry[]): Promise<TableExportData> => {
      ...
      // 1. Собираем все ID ссылок, которые нужно разрешить в имена. Регистратора
      //    среди них нет: его представление приходит готовым полем строки
      //    (`recorderDocumentName`) и уходит в выгрузку общей веткой — дозапрос
      //    за документом дублировал бы то же значение.
