...
/**
 * Цель расшифровки строки LEDGER-отчёта (SCRUM-370 блок В). Резолвит сервер:
 * тип объекта меняется от строки к строке, `groupRefId` для расшифровки не
...
export interface ReportResultDto {
   ...
   * может быть на другом языке — Accept-Language / пер-отчётный тумблер).
   * Отсутствие ⇒ фолбэк на язык приложения (обратная совместимость).
   */
  language?: string
