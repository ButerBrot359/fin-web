...
/**
 * Логины, под которыми на ЭТОМ устройстве уже входили, — запасной источник для выпадашки поля
 * «Пользователь».
 *
 * <b>Основной источник — сервер</b> (`GET /api/auth/selection-list`, паритет со списком выбора
 * 1С). Эти логины показываются, только когда серверный список пуст или недоступен: браузер
 * помнит ровно то, что и так знает про себя, и терять эту подсказку из-за отказа сервера
 * незачем. Общая машина покажет тех, кто на ней работал, и никого больше.
 */
