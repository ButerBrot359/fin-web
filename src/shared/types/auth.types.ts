...
export interface CurrentUser {
  ...
  /**
   * Учётная запись принимает обращения в живую поддержку.
   *
   * По этому признаку решается, что показать в шапке: очередь обращений агенту или кнопку
   * «Позвонить в поддержку» остальным. Признак справочный, не разрешающий — право
   * проверяется сервером при подключении к обращению.
   */
